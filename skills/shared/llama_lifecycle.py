...
# ---- 共享底层（端口检测 / 健康检查） ----
def _ensure_shared_import():
    """确保 skills.shared 包可导入"""
...
def _wait_for_llama_ready(port=8080, timeout=180, label="[LLAMA]"):
    ...
    3. /v1/completions 返回有效响应（最多 30s）
    ...
    while time.time() < deadline:
        ...
        print(f"{label} 超时：端口 {port} 在 {timeout}s 内未打开", file=sys.stderr, flush=True)
        return False
    ...
    # 阶段 3: /v1/completions 功能验证
    comp_deadline = min(time.time() + 30, deadline + 10)
    ...
    while time.time() < comp_deadline:
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status == 200:
                    print(f"{label} /v1/completions 就绪", file=sys.stderr, flush=True)
                    return True
        except Exception:
            pass
        time.sleep(0.5)

    print(f"{label} /v1/completions 超时", file=sys.stderr, flush=True)
    return False
...
def stop_llama(port=8080):
    ...
    for i in range(30):
        if not _port_open("127.0.0.1", port, timeout=1):
            print(f"[LLAMA] 端口 {port} 已释放 ({i+1}s)", file=sys.stderr, flush=True)
            return True
        time.sleep(0.5)

    print(f"[LLAMA] 警告：端口 {port} 仍未释放，继续执行", file=sys.stderr, flush=True)
    return True


def start_llama(port=8080, exe_path=None, model_path=None, log_dir=None, timeout=180):
    """
    ...
    proc = subprocess.Popen(
        args,
        stdout=open(os.path.join(log_dir, "llama-out.log"), "ab") if log_dir else subprocess.DEVNULL,
        stderr=open(os.path.join(log_dir, "llama-err.log"), "ab") if log_dir else subprocess.DEVNULL,
    )

    print(f"[LLAMA] 已启动，PID={proc.pid}，等待端口 {port}...", file=sys.stderr, flush=True)
...
def register_cleanup_handlers(lock_file, llama_port=None, restart_script=None):
    """
    ...
    """
    global _cleanup_lock_file, _cleanup_restart_script, _cleanup_llama_port, _lock_released
